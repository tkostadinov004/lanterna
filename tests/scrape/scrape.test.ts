...
describe('scrape_presentation', () => {
    ...
    it('should scrape successfully without OCR and merge the result', async () => {
        ...
        expect(mock_page.screenshot).toHaveBeenCalledWith({ type: 'webp', quality: 100 });
    ...
    it('should handle missing script tag)', async () => {
        (axios.get as jest.Mock).mockResolvedValue({ data: '<html><body>No scripts here</body></html>' });  

        await expect(scrape_presentation(mock_presentation_url, undefined, { ocr: false }, { separate: false }))
            .rejects
            .toThrow(Error);
    });
});